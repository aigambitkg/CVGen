...
class CVGenLauncher:
    ...
    def set_window_icon(self):
        ...
        try:
            ...
            if sys.platform == "win32":
                ...
                icon_path = (
                    Path(__file__).parent.parent.parent / "desktop" / "resources" / "icon.ico"
                )
                if icon_path.exists():
    ...
    def create_icon_image(self):
        ...
        try:
            ...
            colors = [(0, 200, 255, 180), (0, 128, 255, 140), (0, 100, 200, 100)]

            for i, color in enumerate(colors):
                radius = 30 - (i * 10)
                draw.ellipse(
                    [(32 - radius, 32 - radius), (32 + radius, 32 + radius)], outline=color, width=2
                )
    ...
    def build_ui(self):
        ...
        title_label = tk.Label(
            title_frame, text=APP_NAME, font=title_font, fg=accent_color, bg=bg_color
        )
        title_label.pack()
        ...
        self.status_indicator = tk.Canvas(
            status_frame, width=12, height=12, bg=bg_color, highlightthickness=0
        )
        self.status_indicator.pack(side=tk.LEFT, padx=(0, 8))

        self.status_text = tk.Label(
            status_frame, text="Offline", font=("Helvetica", 10), fg="#ff6b6b", bg=bg_color
        )
        ...
        port_label = tk.Label(
            port_frame, text="Port:", font=("Helvetica", 10), fg=fg_color, bg=bg_color
        )
        port_label.pack(side=tk.LEFT, padx=(0, 10))

        self.port_var = tk.StringVar(value=str(DEFAULT_PORT))
        port_entry = ttk.Spinbox(
            port_frame, from_=1024, to=65535, textvariable=self.port_var, width=10, state="readonly"
        )
        ...
        log_label = tk.Label(
            log_frame, text="Log:", font=("Helvetica", 10, "bold"), fg=fg_color, bg=bg_color
        )
    ...
    def _start_backend_thread(self):
        ...
        try:
            ...
            self.backend_process = threading.Thread(target=self._run_uvicorn, daemon=True)
    ...
    def _run_uvicorn(self):
        ...
        try:
            ...
            uvicorn.run(app, host=self.host, port=self.port, log_level="info", access_log=False)
        except Exception as e:
    ...
    def on_closing(self):
        ...
        if self.backend_ready:
            response = messagebox.askyesnocancel(
                "Confirm Exit", "The backend is still running. Stop it before exiting?"
            )
            if response is None:
                return
            if response:
